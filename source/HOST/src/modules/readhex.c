...
unsigned long read_blockx2(char * fname,unsigned long start_addr,unsigned long block_len,unsigned long dest_addr)
{
	char line[210];
	int bytes,index,rtype,ii;
	unsigned long addr,ioffset;
	...
	{
		memory[dest_addr+rbytes]=0xff;
		memory_used[dest_addr+rbytes]=0;
	}
			...
			while(fgets(line,200,datei))
			{
				if(!(strncmp(line,"S1", 2)))
				{
					...
					for(index=0;index<bytes;index++)
					{
						...
						{
							memory[addr-start_addr+dest_addr]=dbyte;
							memory_used[addr-start_addr+dest_addr]=1;
							if(addr < loaddr) loaddr=addr;
							if(addr > hiaddr) hiaddr=addr;
						}
						addr++;	
...
unsigned long read_block_used(unsigned long start_addr,unsigned long block_len,unsigned long dest_addr)
{
	unsigned long res;
	...
	if(file3_found==2) res+=read_blockx2(sfile3,start_addr,block_len,dest_addr);
	if(file4_found==2) res+=read_blockx2(sfile4,start_addr,block_len,dest_addr);
	return res;
}
